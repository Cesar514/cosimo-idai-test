...
- **Access**: Use `gget` skill or web.run
...
- **Versioning**: Record preprint version and date
- **Publication status**: Check CrossRef/OpenAlex for a peer-reviewed version; cite the published article if available
...
### Preprint Publication Checks (CrossRef/OpenAlex)

When using preprints, confirm if a peer-reviewed version exists:
- **CrossRef**: search by DOI or title; compare authors and abstract
- **OpenAlex**: search by title to find linked published records
- **Action**: If a published version exists, cite the final version and note that it supersedes the preprint

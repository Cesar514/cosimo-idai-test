...
\paragraph{Repeated-run protocol.}
The benchmark harness supports configurable repeated-run timing via \texttt{--repeats} and \texttt{--warmup} (see Section~\ref{sec:method}).
With \(\texttt{--repeats} > 1\), each planner-maze pair is timed independently \(R\) times (after \(W\) discarded warm-up runs), and two additional artifacts are emitted: per-(planner, maze) aggregate statistics (mean, median, standard deviation) in \texttt{benchmark\_repeat\_stats.csv}, and a rank-stability report (\texttt{benchmark\_rank\_stability.md}) containing pairwise Spearman \(\rho\) across repeat-round rankings.
The current committed results use the default \(\texttt{--repeats}~1\) single-measurement protocol; timing claims should therefore be interpreted as exploratory.
Running with \(\texttt{--repeats}~\ge~5\) and \(\texttt{--warmup}~1\) is recommended before drawing inferential conclusions about runtime ordering.

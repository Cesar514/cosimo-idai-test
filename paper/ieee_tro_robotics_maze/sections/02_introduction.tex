...
This paper contributes three bounded, repository-grounded results focused on integration-level novelty:
\begin{itemize}
    \item \textbf{Integration-level infrastructure:} a deterministic planner-evaluation runtime that couples seed propagation, heterogeneous planner I/O normalization, and geometric path validation into one auditable repository workflow.
    \item \textbf{Standardized evaluation protocol:} a shared-success comparability policy with deterministic lexicographic ranking by success rate and efficiency metrics, designed to reduce reporting bias in static-grid comparisons.
    \item \textbf{Traceable evidence snapshot:} an executed benchmark (12 planners, 50 mazes) with trial-level artifacts and deterministic simulation regression checks, serving as a reproducibility baseline.
\end{itemize}
